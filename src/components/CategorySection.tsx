...
export const CategorySection: React.FC<CategorySectionProps> = ({
  ...
  const handleStartAll = () => {
    Alert.alert('Start All Timers', `Start all timers in "${category.name}"?`, [
  ...
  const handlePauseAll = () => {
    Alert.alert('Pause All Timers', `Pause all timers in "${category.name}"?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Pause All',
        onPress: () => {
  ...
  const handleResetAll = () => {
    Alert.alert('Reset All Timers', `Reset all timers in "${category.name}"?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Reset All',
        onPress: () => {
  ...
  const runningCount = category.timers.filter(
    t => t.status === 'running',
  ).length;
  const completedCount = category.timers.filter(
  ...
  return (
    <View style={CategoryStyle.container}>
      ...
      >
        <View style={CategoryStyle.headerLeft}>
          ...
          <View style={CategoryStyle.statsContainer}>
            <Text style={CategoryStyle.statsText}>
              {category.timers.length} timer
              {category.timers.length !== 1 ? 's' : ''}
            </Text>
            {runningCount > 0 && (
              <Text style={CategoryStyle.runningText}>
                {' '}
                • {runningCount} running
              </Text>
            )}
            {pausedCount > 0 && (
              <Text style={CategoryStyle.pausedText}>
                {' '}
                • {pausedCount} paused
              </Text>
            )}
            {completedCount > 0 && (
              <Text style={CategoryStyle.completedText}>
                {' '}
        ...
        <Icon
          name={category.isExpanded ? 'expand-less' : 'expand-more'}
          size={24}
          color="#8E8E93"
        />
      </TouchableOpacity>
        ...
        <>
          {category.timers.length > 1 && (
            <View style={CategoryStyle.bulkActions}>
              <TouchableOpacity
                style={[CategoryStyle.bulkButton, CategoryStyle.startAllButton]}
                onPress={handleStartAll}
              >
                <Text style={CategoryStyle.bulkButtonText}>Start All</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[CategoryStyle.bulkButton, CategoryStyle.pauseAllButton]}
                onPress={handlePauseAll}
              >
                <Text style={CategoryStyle.bulkButtonText}>Pause All</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[CategoryStyle.bulkButton, CategoryStyle.resetAllButton]}
